...
describe('Lottery', () => {
  ...
  describe('when user opens a lottery', () => {
    it('reverts if user is not owner', async () => {
      await assertRevert(Lottery.connect(user).startLottery(), 'Ownable: caller is not the owner');
    });

    it('reverts if Lottery state was not in CLOSED state', async () => {
      await Lottery.startLottery();
      await assertRevert(Lottery.startLottery(), "can't start the lottery yet");
    });
  ...
  describe('when owner ends the lottery', () => {
    it('reverts if lottery is not OPEN yet', async () => {
      await assertRevert(Lottery.endLottery(), 'Lottery is not open yet');
    });

    it("reverts if lottery contract doesn't have enough LINK", async () => {
      tx = await Lottery.startLottery();
      await assertRevert(Lottery.endLottery(), 'Not enough link tokens');
    });
